...
void liberarGradientes(Gradiente_t *grad){
    ...
    if (grad) {
        ...
        //Verifica se cada uma das estruturas interna não são nulas.
        //Se não forem, desaloca elas da memória.
        if (grad->aux_A[0])
            free(grad->aux_A[0]);
        if (grad->aux_A)
            free(grad->aux_A);
...
Gradiente_t* alocarGradiente(unsigned int n) {
    ...
    // Se foi alocado espaço para o ponteiro de gradientes,
    if (grad) {

        // Aloca espaço para as estruturas internas do gradiente.
        // Caso alguma delas não seja alocada, desaloca as estruturas que foram alocadas anteriormente
